...
int main(){
    ...
    for (int i=1;i<=m;i++){
	int x;scanf("%d%d",&x,&Om[i]);
	Qm[x].push_back(i);
    }
    Sn[0]=Srev[0]=((StackData){1,0});
    revtop=top=0;
    for (int i=n;i>=1;i--){
	...
	Sn[++top]=now;Pre[i]=(Pre[now.l-1]+Calc(now.l,now.r))%Mod;
    }
    printf("%d\n",Pre[n]);
...
StackData operator + (StackData A,StackData B){
    A.l=min(A.l,B.l);A.r=max(A.r,B.r);return A;
}
int Calc(int l,int r){
...
int brute(int l,int r){
    int sum=0;for (int i=l;i<=r;i++) sum=(sum+Seq[i])%Mod;
