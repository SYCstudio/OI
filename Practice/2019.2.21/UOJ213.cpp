...
{
	int lg=Lg[r-l+1];
	return max(ST[lg][l],ST[lg][r-pw(lg)+1],cmp);
}
void Divide(int l,int r)
{
	//cout<<"Divide:"<<l<<" "<<r<<endl;
	if (l==r) {
		Sb[1]=(Sb[1]+Seq[l]%Mod)%Mod;
		Sb[2]=(Sb[2]-Seq[l]%Mod+Mod)%Mod;
	...
	Sk[mx]=(Sk[mx]+k)%Mod;
	Sk[sz+1]=(Sk[sz+1]-k+Mod)%Mod;
	Sb[mx]=(Sb[mx]+b)%Mod;
	Sb[sz+1]=(Sb[sz+1]-b+Mod)%Mod;
	b=1ll*Seq[p]%Mod*mn%Mod;
	Sb[mn]=(Sb[mn]+b)%Mod;
	Sb[mx]=(Sb[mx]-b+Mod)%Mod;
	k=Seq[p]%Mod;
	Sk[1]=(Sk[1]+k)%Mod;
	Sk[mn]=(Sk[mn]-k+Mod)%Mod;
	//for (int i=1,s1=0,s2=0;i<=n;i++) s1=(s1+Sk[i])%Mod,s2=(s2+Sb[i]%Mod),cout<<(1ll*i*s1%Mod+s2)%Mod<<" ";cout<<endl;
	if (l<p) Divide(l,p-1);
	if (p<r) Divide(p+1,r);
	return;
