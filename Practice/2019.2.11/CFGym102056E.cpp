...
int main()
{
	freopen("in","r",stdin);
	Get();
	for (int j=-Num; j<=Num; j++) for (int b=0; b<=1; b++) F[maxN+j][b]=(Dp[now][maxN+j][b][0]+Dp[now][maxN+j][b][1])%Mod;
	Get();
	for (int j=-Num; j<=Num; j++) for (int b=0; b<=1; b++) G[maxN+j][b]=((Dp[now][maxN+j][b][0]+G[maxN+j-1][b])%Mod+Dp[now][maxN+j][b][1])%Mod;
	int Ans=0;
