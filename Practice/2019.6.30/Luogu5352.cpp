...
public:
	...
	int zero[maxB];
	int Xor(){
		int sum=0;for (int i=0;i<maxB;i++) if ((siz-zero[i])&1) sum|=pw(i);
		return sum;
	}
	int And(){
		int sum=0;for (int i=0;i<maxB;i++) if (siz&&zero[i]==0) sum|=pw(i);
		return sum;
	}
	int Or(){
		int sum=0;for (int i=0;i<maxB;i++) if (siz-zero[i]) sum|=pw(i);
		return sum;
	}
	ll Sum(){
		ll sum=0;for (int i=0;i<maxB;i++) sum=sum+1ll*pw(i)*(siz-zero[i]);
		return sum;
	}
};
...
int main(){
	...
	for (int i=1;i<=n;i++){
		int key;scanf("%d",&key);Xor(i,key);S[i].siz=1;
	}
	while (Q--){
		char opt;int x,y;scanf(" %c%d%d",&opt,&x,&y);
		if (opt=='L') Link(x,y);
		if (opt=='C') Cut(x,y);
		if (opt=='U'){
			int key;scanf("%d",&key);Split(x,y);Xor(y,key);
		}
		if (opt=='A'){
			Split(x,y);printf("%d\n",S[y].And());
		}
...
void Splay(int x){
	static int St[maxN];int now=x,top=1;St[1]=now;
...
void Cut(int x,int y){
	Split(x,y);S[y].ch[0]=S[x].fa=0;Update(x);Update(y);
	return;
}
