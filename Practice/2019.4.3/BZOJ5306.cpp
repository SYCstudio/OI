...
int main(){
    int n,m,S;
    scanf("%d%d%d",&n,&m,&S);int X=min(m,n/S);int mx=max(n,m);

    Fc[0]=Ifc[0]=1;for (int i=1;i<=mx;i++) Fc[i]=1ll*Fc[i-1]*i%Mod;
    Ifc[mx]=QPow(Fc[mx],Mod-2);for (int i=mx-1;i>=1;i--) Ifc[i]=1ll*Ifc[i+1]*(i+1)%Mod;
    ...
    for (int i=0;i<=X;i++) if (i&1) B[i]=Mod-Ifc[i];else B[i]=Ifc[i];

    reverse(&B[0],&B[X+1]);int len=1;while (len<=X+X) len<<=1;

    NTT(A,len,1);NTT(B,len,1);for (int i=0;i<len;i++) A[i]=1ll*A[i]*B[i]%Mod;NTT(A,len,-1);
    int Ans=0;
    for (int i=X;i<=X+X;i++){
        int w;scanf("%d",&w);Ans=(Ans+1ll*w*A[i]%Mod*Ifc[i-X]%Mod)%Mod;
    }
    printf("%d\n",Ans);return 0;
}
int QPow(int x,int cnt){
    ...
    for (int i=1;i<N;i<<=1){
        ...
        for (int j=0;j<N;j+=(i<<1))
            for (int k=j,w=1;k<i+j;k++,w=1ll*w*dw%Mod){
                ...
                P[k]=X+Y;if (P[k]>=Mod) P[k]-=Mod;
                P[k+i]=X-Y;if (P[k+i]<0) P[k+i]+=Mod;
            }
    }
    if (opt==-1){
