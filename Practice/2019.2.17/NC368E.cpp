...
Data operator + (Data A,Data B)
{
	...
	if (!In(A.u,A.v,LCA(B.u,B.v))&&!In(B.u,B.v,LCA(A.u,A.v))) {
		R.opt=1;
		return R;
	};
	...
	//cout<<R.u<<" "<<R.v<<endl;
	vector<int> P,Q;
	if (Dst(A.a,R.u)<=Dst(A.a,R.v)&&P.size()<2) P.push_back(A.a);
	else Q.push_back(A.a);
	if (Dst(A.b,R.u)<=Dst(A.b,R.v)&&P.size()<2) P.push_back(A.b);
	else Q.push_back(A.b);
	if (Dst(B.a,R.u)<=Dst(B.a,R.v)&&P.size()<2) P.push_back(B.a);
	else Q.push_back(B.a);
	if (Dst(B.b,R.u)<=Dst(B.b,R.v)&&P.size()<2) P.push_back(B.b);
	else Q.push_back(B.b);
...
void Modify(int x,int l,int r,int p,int u,int v)
{
	...
	int mid=(l+r)>>1;
	if (p<=mid) Modify(ls,l,mid,p,u,v);
	else Modify(rs,mid+1,r,p,u,v);
	S[x]=S[ls]+S[rs];
	return;
}
Data Query(int x,int l,int r,int ql,int qr)
