...
public:
	...
	int LCA(int u,int v)
	{
		...
		for (int i=maxB-1; i>=0; i--) if (ST[i][u]&&Dep[ST[i][u]]>=Dep[v]) u=ST[i][u];
		if (u==v) return u;
		for (int i=maxB-1; i>=0; i--) if (ST[i][u]&&ST[i][v]&&ST[i][u]!=ST[i][v]) u=ST[i][u],v=ST[i][v];
		return ST[0][u];
	}
	int GET(int u,int cnt)
	{
		...
		int now=u;
		for (int i=maxB-1; i>=0; i--) if (ST[i][now]&&Mark[ST[i][now]]>cnt) now=ST[i][now];
		now=ST[0][now];
...
int main()
{
	...
	Bfs(1);
	S=1;
	for (int i=1; i<=n; i++) if (Dst[S]<Dst[i]) S=i;
	Bfs(S);
	T=1;
	for (int i=1; i<=n; i++) if (Dst[T]<Dst[i]) T=i;
	//cout<<S<<" "<<T<<endl;
	T1.Init(S);
	T2.Init(T);
	int lstans=0;
