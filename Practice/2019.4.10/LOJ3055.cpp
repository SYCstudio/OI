...
void dfs(int u,int d,int len)
{
	...
	for (int i=0,sz=T[u].size(); i<sz; i++) {
		...
		int prert=rt[d][c],prelen=mxlen[d][c];
		mxlen[d][c]=max(mxlen[d][c],l);
		memcpy(rt[d+1],rt[nxt],sizeof(rt[nxt]));
		memcpy(mxlen[d+1],mxlen[nxt],sizeof(mxlen[nxt]));
