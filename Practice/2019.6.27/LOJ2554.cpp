...
int main(){
    F[0]=1;F[1]=2;Divide(2,50000);
    int Case;scanf("%d%d",&Case,&n);
    while (Case--){
	for (int i=1;i<=n;i++) scanf("%d",&L[i]),L[i]=i-L[i]+1;
	if (L[n]!=1){
	    puts("0");continue;
	}
	top=0;int Ans=1;bool flag=1;
	for (int i=1;i<=n;i++){
	    int son=0;
	    while (top){
	    ...
	    if (flag==0) break;
	    Ans=1ll*Ans*F[son]%Mod;St[++top]=i;
	}
	if (flag) printf("%d\n",Ans);
    }
    return 0;
...
void Divide(int l,int r){
    ...
    if (l>2){
	int up=min(l-1,r-l);N=1;while (N<=up-1+mid-l+1) N<<=1;
