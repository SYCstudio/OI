...
int main()
{
	...
	for (int i=1; i<n; i++) {
		...
		To[u].push_back((VData) {
			v,a,b
		});
		To[v].push_back((VData) {
			u,a,b
		});
	}
	dfs_init(1,1);
	for (int i=1,sz; i<=n; i++)
		if (Sn[i].size()<=2)
			...
			int ls=++n,rs=++n;
			Add_Edge(i,ls,make_pair(0,0));
			Add_Edge(i,rs,make_pair(0,0));
...
void dfs_get(int u,int fa,int opt,ll a,ll b)
{
	if (tot[opt]==0||(C[opt][tot[opt]].x!=a)||(C[opt][tot[opt]].y!=b)) C[opt][++tot[opt]]=((Point) {
		a,b
	});
	for (int i=Head[u]; i!=-1; i=Next[i])
...
void Divide(int start,int size)
{
	...
	int top=1,p0=1,p1=1,up0=0,up1=0;
	Bp[1]=C[0][1]+C[1][1];
	C[0][tot[0]+1]=C[0][1];
	C[1][tot[1]+1]=C[1][1];
	...
	for (int i=1; i<=top; i++) Ans[++anstop]=Bp[i]+((Point) {
		W[rte].first,W[rte].second
	});
	Divide(u,Sz[u]);
	Divide(v,size-Sz[u]);
	return;
...
void Convex(Point *P,int &T)
{
	...
	for (int i=top; i>=1; i--) P[i]=St[top-i+1];
	T=top;
	/*
	Point base=P[1];
