...
int main(){
	...
	if (inst[Eular[i]]==1){
	    ++cnt;Lf[cnt]=seqcnt+1;Ans[++seqcnt]=Eular[i];
	    while (top&&St[top]!=Eular[i]) --inst[St[top]],Ans[++seqcnt]=St[top--];
	    Ans[++seqcnt]=Eular[i];Rh[cnt]=seqcnt;
	}
	else{
	    ++inst[Eular[i]];St[++top]=Eular[i];
	}
...
void Add_Edge(int u,int v){
    ...
    Nt[++ecnt]=Hd[v];Hd[v]=ecnt;V[ecnt]=u;
    ++Dg[u];++Dg[v];return;
}
void dfs(int u){
    vis[u]=1;
