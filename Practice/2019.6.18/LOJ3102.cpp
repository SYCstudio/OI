...
int main(){
    ...
    Mic[0]=1;for (int i=1;i<maxN;i++) Mic[i]=Mod-Mic[i-1];
	...
	sum+=Ts[mi];for (int i=0;i<=sum;i++) Poly[i]=bp[i],bp[i]=0,iPoly[i]=0;
...
void dfs(int u,int fa){
    Sz[u]=1;dp[u][1][2]=1;dp[u][0][0]=1;dp[u][0][1]=1;
	...
	Sz[u]+=Sz[v];for (int j=0;j<=Sz[u];j++) for (int c=0;c<=2;c++) dp[u][j][c]=dpg[j][c],dpg[j][c]=0;
    }
    return;
}
