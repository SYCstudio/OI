...
int main(){
	...
	sort(&Sorter[1],&Sorter[sz+1]);int bsz=0,cnt=0;
	//for (int i=1;i<=sz;i++) cout<<"("<<Sorter[i].ft<<" "<<Sorter[i].sd<<") ";cout<<endl;
	for (int l=1,r;l<=sz;l=r+1){
	    r=l;while (r<sz&&Sorter[r+1].ft==Sorter[l].ft) ++r;
	    if (Sorter[l].ft<=sqr) for (int i=l;i<=r;i++) if (bsz<maxB&&Insert(Get(Sorter[i].sd))) ++bsz,++cnt;else;
	    else{
		++cnt;bitset<maxB> F=Get(Sorter[l].sd);
...
bitset<maxB> Get(int key){
    bitset<maxB> R;if (Mx[key]>sqr) key/=Mx[key];
