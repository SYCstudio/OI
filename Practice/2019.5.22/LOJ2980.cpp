...
public:
    ...
    Mat(int opt){
	memset(M,0,sizeof(M));M[0][0]=M[1][1]=M[2][2]=M[3][3]=1;
	if (opt==0) M[1][0]=1;
	if (opt==1) M[2][1]=1;
	if (opt==2) M[0][2]=1;
	return;
...
int main(){
    ...
    scanf("%d",&Q);
    while (Q--){
	int opt,l,r,v;scanf("%d%d%d",&opt,&l,&r);
	if (opt<=3) Modify(1,1,n,l,r,Mat(opt-1));
	else if (opt<=6){
	    scanf("%d",&v);Modify(1,1,n,l,r,Mat(opt-4,v));
	}
	else{
...
void Build(int x,int l,int r){
	...
	S[x].M[0][0]=a;S[x].M[0][1]=b;S[x].M[0][2]=c;S[x].M[0][3]=1;
	return;
    }
...
void Modify(int x,int l,int r,int ql,int qr,Mat M){
    if (l==ql&&r==qr){
	S[x]=S[x]*M;T[x]=T[x]*M;return;
    }
    S[ls]=S[ls]*T[x];T[ls]=T[ls]*T[x];
    S[rs]=S[rs]*T[x];T[rs]=T[rs]*T[x];
    T[x]=Mat(5);int mid=(l+r)>>1;
