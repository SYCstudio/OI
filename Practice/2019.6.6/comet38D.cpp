...
public:
	...
	{
		sm=k;
		for (int i=maxB-1; i>=0; i--) B.B[i]=0;
		B.Insert(k);
		return;
	}
};
...
int main()
{
	...
	while (m--) {
		...
		if (opt==1) {
			Plus(1,1,n,l,v);
			if (r+1<=n) Plus(1,1,n,r+1,v);
		} else {
			Base Ans;
			if (l<r) Ans=Query(1,1,n,l+1,r);
...
void Plus(int x,int l,int r,int p,int k)
{
	...
	int mid=(l+r)>>1;
	if (p<=mid) Plus(ls,l,mid,p,k);
	else Plus(rs,mid+1,r,p,k);
	Update(x);
	return;
}
int Query(int x,int l,int r,int p)
