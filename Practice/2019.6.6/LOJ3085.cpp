...
class Watcher{
public:
    double x,l,r;int opt;
    Watcher(){
	x=l=r=0;opt=0;return;
    }
    Watcher(double a,double b,double c,int o){
	x=a;l=b;r=c;opt=o;return;
    }
};
...
int main(){
    ...
    //Work C
    for (int i=1;i<=crxcnt;i++) Crx[i]=(Point){Crx[i].x+Crx[i].y,Crx[i].x-Crx[i].y};
    sort(&Crx[1],&Crx[crxcnt+1],xcmp);
    int K,wcnt=0;scanf("%d",&K);
    srty[++numy]=1e18;
    for (int i=1;i<=K;i++){
	double dx,dy,r,x,y;scanf("%lf%lf%lf",&dx,&dy,&r);
	x=dx+dy;y=dx-dy;
	Wch[++wcnt]=(Watcher){x-r,y-r,y+r+eps,1};
	Wch[++wcnt]=(Watcher){x+r+eps,y-r,y+r+eps,-1};
	srty[++numy]=y-r;srty[++numy]=y+r+eps;
    }
    sort(&srty[1],&srty[numy+1]);sort(&Wch[1],&Wch[wcnt+1],wcmp);
...
#define Dct(x) (lower_bound(&srty[1],&srty[numy+1],x)-srty)
	...
	r=l;while (r<wcnt&&Wch[r+1].x==Wch[l].x) ++r;
	for (int i=l;i<=r;i++){
...
#undef Dct
    ...
    baseans=baseans*C;
    for (int i=1;i<=crxcnt;i++) Crx[i]=(Point){(Crx[i].x+Crx[i].y)/2,(Crx[i].x-Crx[i].y)/2};
	...
	if (Id[i]!=i){
	    int p=uId[i];++c;
	    swap(Id[i],Id[p]);uId[Id[i]]=i;uId[Id[p]]=p;
	}
    Ans2=A*c+B*(crxcnt-c);
    
    Ans1+=baseans;Ans2+=baseans;
...
Point GetCross(int i,int j){
    ...
    return (Point){XST+d,P[i].ft+d*ki};
}
bool xcmp(Point A,Point B){
