...
int main(){
    scanf("%d%d",&n,&K);N=pw(K);scanf("%d%d%d",&X,&Y,&Z);int suma=0;
    for (int i=1;i<=n;i++){
	int a,b,c;scanf("%d%d%d",&a,&b,&c);suma^=a;
	++A[a^b];++B[a^c];++C[b^c];
    }
    ...
    for (int i=0;i<N;i++){
	int a,b,c,d;
	a=((n+A[i])%Mod+(B[i]+C[i])%Mod)%Mod;a=1ll*a*inv4%Mod;
	b=((n+A[i])%Mod-(B[i]+C[i])%Mod+Mod)%Mod;b=1ll*b*inv4%Mod;
	c=((n+B[i])%Mod-(A[i]+C[i])%Mod+Mod)%Mod;c=1ll*c*inv4%Mod;
	d=((n+C[i])%Mod-(A[i]+B[i])%Mod+Mod)%Mod;d=1ll*d*inv4%Mod;
	F[i]=1ll*QPow(S1,a)*QPow(S2,b)%Mod*QPow(S3,c)%Mod*QPow(S4,d)%Mod;
    }
    FWT(F,N,-1);
    for (int i=0;i<N;i++) printf("%d ",F[i^suma]);return 0;
}
int QPow(int x,int cnt){
    int ret=1;
    while (cnt){
