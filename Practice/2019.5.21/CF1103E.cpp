...
int main(){
    W[0].a[0]=1;W[1].a[1]=1;for (int i=2;i<10;i++) W[i]=W[i-1]*W[1];
    int n;scanf("%d",&n);
    for (int i=1;i<=n;i++){
	int x;scanf("%d",&x);++P[x].a[0];
    }
    FWT(P,1);for (int i=0;i<maxN;i++) P[i]=QPow(P[i],n);FWT(P,9);
...
Num operator * (Num A,Num B){
    ull tmp[10];memset(tmp,0,sizeof(tmp));
