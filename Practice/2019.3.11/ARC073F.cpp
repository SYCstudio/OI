...
public:
	...
	void Cmpmin(int x,int l,int r,int p,ll key)
	{
		...
		int mid=(l+r)>>1;
		PushDown(x);
		if (p<=mid) Cmpmin(ls,l,mid,p,key);
		else Cmpmin(rs,mid+1,r,p,key);
		Update(x);
		return;
	}
...
int main()
{
	...
	for (int i=1; i<=Q; i++) {
		...
		T1.RangePlus(1,1,n,1,n,abs(p-P[i-1]));
		T2.RangePlus(1,1,n,1,n,abs(p-P[i-1]));
		T1.Cmpmin(1,1,n,P[i-1],mn+P[i-1]);
		T2.Cmpmin(1,1,n,P[i-1],mn-P[i-1]);
	}
