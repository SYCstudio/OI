...
int main(){
    ...
    Build(1,1,n);
    while (Q--){
	int type,l,r,d;scanf("%d%d%d",&type,&l,&r);
	if (type==1){
	    scanf("%d",&d);HASH::Modify(d,l,r);
	    Modify(1,1,n,l,r);
	}
	else{
	    int q=-1;Query(1,1,n,l,r,r,q);
	    printf("%d\n",q);
	}
    }
    return 0;
...
void Outp(int x,int l,int r){
    cout<<x<<"["<<l<<" "<<r<<"]:";for (int i=0;i<P[x].size();i++) cout<<P[x][i]<<" ";cout<<endl;
    if (l==r) return;
    int mid=getmid(l,r);Outp(ls,l,mid);Outp(rs,mid+1,r);
    return;
}
#undef ls
#undef rs
...
namespace HASH{
    void Init(){
	pwb[0]=1;for (int i=1;i<maxN;i++) pwb[i]=pwb[i-1]*base%Mod;
	pws[1]=pwb[0];for (int i=2;i<maxN;i++) pws[i]=(pws[i-1]+pwb[i-1])%Mod;
	for (int i=1;i<=n;i++){
	    Be[i]=i/SQRT+1;Br[Be[i]]=i;
	...
	if (Be[l]==Be[r]){
		...
		(bsuf[i]+=right)%=Mod;right=right*pwb[Br[i]-Bl[i]+1]%Mod;
	    }
	}
	else{
		...
		(bplus[i]+=X)%=Mod;(bsuf[i]+=right)%=Mod;
		numplus[i]+=x;
