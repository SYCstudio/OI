...
int main(){
    ...
    scanf("%d%d",&n,&m);int idcnt=0;for (int i=1;i<=n;i++) for (int j=1;j<=m;j++) Id[i][j]=++idcnt;
    S=++idcnt;T=++idcnt;scanf("%d%d%d%d",&sx,&sy,&tx,&ty);
    for (int i=1;i<=n;i++) for (int j=1;j<=m;j++) scanf("%d",&Mt[i][j]);
    for (int i=1;i<=n;i++)
        for (int j=1;j<=m;j++){
            pair<int,int> Vr[8];int vcnt=0;
            for (int f=0;f<4;f++){
    ...
    Add_Edge(S,Id[sx][sy],inf);Add_Edge(Id[tx][ty],T,inf);
    int flow=0;
    while (bfs()){
