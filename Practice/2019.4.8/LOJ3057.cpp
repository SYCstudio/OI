...
int main(){
    scanf("%d%d%d",&n,&m,&Q);scanf("%s",Col+1);
    for (int i=1;i<=m;i++){
    ...
    while (Q--){
        int u,v;scanf("%d%d",&u,&v);
        if (Ans[u][v]) puts("YES");else puts("NO");
    }
    return 0;
}
void dfs_same(int u,int fa){
    for (int i=0,sz=E[u].size();i<sz;i++){
        int v=E[u][i];if (v==fa||Col[v]!=Col[u]) continue;
        if (Mark[v]==-1){
            Add(u,v);
            Mark[v]=Mark[u]^1;dfs_same(v,u);
        }
...
void dfs_unsame(int u,int fa){
    for (int i=0,sz=E[u].size();i<sz;i++){
        int v=E[u][i];if (v==fa||Col[v]==Col[u]) continue;
        if (!vis[v]){
            vis[v]=1;Add(u,v);dfs_unsame(v,u);
        }
    }
    return;
}
void Add(int u,int v){
    ...
    if (Col[v]=='1') W[u].push_back(v);else B[u].push_back(v);
    if (Col[u]=='1') W[v].push_back(u);else B[v].push_back(u);
    return;
}