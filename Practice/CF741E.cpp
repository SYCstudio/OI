...
int main(){
	...
	/*
	for (int i=0;i<=L1;i++){
		for (int j=1;j<=Sorter[i];j++) cout<<S[j];
		for (int j=1;j<=L2;j++) cout<<T[j];
		for (int j=Sorter[i]+1;j<=L1;j++) cout<<S[j];
		cout<<endl;
	}
	//*/
    ...
    for (int qi=1;qi<=Q;qi++){
		...
		if (k>=maxSqrt)
			for (int i=x,j=y;(i<=L1)&&(j<=L1);i+=K,j+=K)
				if ((j>=l)&&(i<=r)) Ans[qi]=ChkMin(Ans[qi],GetMin(max(i,l),min(j,r)));
    ...
    for (int i=1;i<=Q;i++) printf("%d ",Ans[i].second);return 0;
}
