...
int main()
{
	...
	for (int qi=1; qi<=Q; qi++) {
		...
		for (int i=1; i<=n; i++) if (i!=y&&i!=x) {
				Plus(G[i][x],1ll*F[i][x]*inv2%Mod);
				Plus(G[i][y],1ll*F[i][x]*inv2%Mod);
				Plus(G[x][i],1ll*F[x][i]*inv2%Mod);
				Plus(G[y][i],1ll*F[x][i]*inv2%Mod);
			}
		for (int i=1; i<=n; i++) if (i!=x&&i!=y) {
				Plus(G[i][y],1ll*F[i][y]*inv2%Mod);
				Plus(G[i][x],1ll*F[i][y]*inv2%Mod);
				Plus(G[y][i],1ll*F[y][i]*inv2%Mod);
				Plus(G[x][i],1ll*F[y][i]*inv2%Mod);
			}
