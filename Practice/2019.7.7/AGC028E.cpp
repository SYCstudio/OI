...
int main(){
	...
	Bodd[i]=-inf;Beven[i]=0;
	if (Seq[i]>mx) Mx[i]=1,mx=Seq[i],++mxcnt;
    }

    for (int i=n;i>=1;i--){
	//cerr<<i<<endl;
	int odd=-inf,even=-inf;Sttop[i]=Rc.size();
	if (Mx[i]){
	...
	if (Seq[i]>=mx0){
		...
		printf("0");mx0=Seq[i];++minus;
	    }
	    else{
		printf("1");if (Seq[i]>=mx1) mx1=Seq[i],--minus;
	    }
	}
	else{
