...
int main(){
    int Type;scanf("%d%d%d",&n,&m,&Type);int RG=n+m;
...
void Build(int x,int l,int r){
    if (l==r){
	S[x].a=Seq[l];S[x].b=S[x].c=1;S[x].d=0;return;
    }
    int mid=(l+r)>>1;
...
void Extend(int x,int l,int r,int p){
    ...
    int mid=(l+r)>>1;
    if (p<=mid) Extend(ls,l,mid,p);else Extend(rs,mid+1,r,p);
    S[x]=S[ls]+S[rs];return;
}
