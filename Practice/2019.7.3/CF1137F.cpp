...
void dfs_init(int u,int fa){
	...
	dfs_init(v,u);
	if (S[v].key>S[u].key){
	    S[u].key=S[v].key;S[u].ch[1]=v;S[u].size=S[v].size+1;
	}
    }
    Plus(S[u].key,1);return;
}
bool Isroot(int x){
...
void Splay(int x){
    static int ST[maxN];int now=x,top=1;ST[1]=x;
...
void Access(int x){
    ...
    while (x){
	Splay(x);
	S[x].ch[1]=0;Update(x);Plus(S[x].key,-S[x].size);
	S[x].ch[1]=lstx;Update(x);
	lstx=x;x=S[x].fa;
    }
    Splay(lstx);Plus(nodecnt,S[lstx].size);
    return;
}
int Query(int x){
    Splay(x);return Sum(S[x].key)-S[S[x].ch[0]].size;
}
void Plus(int p,int k){
    //cout<<"Plus:"<<p<<" "<<k<<endl;
    while (p<=n+Q){
