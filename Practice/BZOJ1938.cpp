...
#define Dsp(x) (lower_bound(&Num[1],&Num[num+1],x)-Num)
#define lson (now<<1)
#define rson (lson|1)
...
int main(){
	...
	sort(&Num[1],&Num[num+1]);num=unique(&Num[1],&Num[num+1])-Num-1;Num[num+1]=1e9+1;
	Label(1,1,num);
	for (int i=1;i<=m;i++)
...
void Calc(int now,int a,int b,int st){
	...
	//As[now]=Get(a,b,S[now].r-st)-Get(a,b,S[now].l-st);
	As[now]=a1-a2;
	cout<<"Calc:["<<S[now].l<<","<<S[now].r<<"] a:"<<a<<" b:"<<b<<" st:"<<st<<" - "<<As[now]<<" "<<a1<<" "<<a2<<endl;
	return;
}
...
ll Get(int a,int b,int n){
	...
	else return 1ll*n*(n+1)/2ll*a-1ll*b*Lgcd(a,0,b,n);
	//cout<<"Get:"<<a<<" "<<b<<" "<<n<<":"<<ret<<endl;
	//return ret;
}
