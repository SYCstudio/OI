...
int main(){
    ...
    while (Case--){
	scanf("%d%d%d%d",&n,&m,&L,&K);int mn=min(min(n,m),L),N=1ll*n*m%Mod*L%Mod;
	...
	MulA[0]=1;for (int i=1;i<=mn;i++) MulA[i]=1ll*MulA[i-1]*A[i]%Mod;
	Inva[mn]=Inv(MulA[mn]);for (int i=mn-1;i>=1;i--) Inva[i]=1ll*Inva[i+1]*A[i+1]%Mod;
