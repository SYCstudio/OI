...
int main(){
    ...
    for (int i=0;i<Q;i++)
	if (!vis[i]){
	    int sz=1,fsz=Mark[i],now=Nxt[0][i];vis[i]=1;
	    while (now!=i) ++sz,fsz+=Mark[now],vis[now]=1,now=Nxt[0][now];
	    Sz[i]=sz;Fsz[i]=fsz;now=Nxt[0][i];
	...
	int now=A[i]%Q;ll c=(T-1-A[i])/P;
	Ans=Ans+Mark[now];
	Ans=Ans+1ll*Fsz[now]*(c/Sz[now]);c=c%Sz[now];
	for (int j=Bit-1;j>=0;j--)
	    if (c&(1<<j)){
		Ans=Ans+Sm[j][now];now=Nxt[j][now];
	    }
    }
    printf("%lld\n",Ans);return 0;
}
