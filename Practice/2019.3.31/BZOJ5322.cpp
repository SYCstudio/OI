...
{
	Fc[0]=Ifc[0]=1;
	for (int i=1; i<maxM; i++) Fc[i]=1ll*Fc[i-1]*i%Mod;
	...
	while (Case--) {
		...
		if (m) {
			if (scnt==0||1ll*sz*Sorter[1]>=m) {
				int v=m/sz,res=m%sz;
				Ans=1ll*Ans*QPow(Fc[v+1],res)%Mod*QPow(Fc[v],sz-res)%Mod;
				...
				while (1) {
					if (lst==scnt) {
						int v=m/sz,res=m%sz;
						Ans=1ll*Ans*QPow(Fc[Sorter[scnt]+v+1],res)%Mod*QPow(Fc[Sorter[scnt]+v],sz-res)%Mod;
						break;
					}
					int dw=Sorter[lst+1]-Sorter[lst];
					if (1ll*sz*dw>=m) {
						int v=m/sz,res=m%sz;
						Ans=1ll*Ans*QPow(Fc[Sorter[lst]+v+1],res)%Mod*QPow(Fc[Sorter[lst]+v],sz-res)%Mod;
