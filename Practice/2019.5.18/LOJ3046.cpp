...
int main(){
    ...
    for (int i=1;i<=m;i++){
	int x,y;scanf("%d%d",&x,&y);if (x==y) continue;
	int lca=LCA(x,y),fa=Fa[lca];
	Modify(rt[x],1,dfncnt,x,1);Modify(rt[x],1,dfncnt,y,1);
	Modify(rt[y],1,dfncnt,x,1);Modify(rt[y],1,dfncnt,y,1);
	Del[fa].pb(x);Del[fa].pb(y);
    }
    dfs2(1);printf("%lld\n",Ans/2);return 0;
}
void dfs1(int u){
...
void Modify(int &x,int l,int r,int p,int key){
    ...
    int mid=(l+r)>>1;
    if (dfn[p]<=mid) Modify(S[x].ls,l,mid,p,key);else Modify(S[x].rs,mid+1,r,p,key);
    Update(x);return;
}
int Merge(int u,int v,int l,int r){
    if (!u||!v) return u+v;
