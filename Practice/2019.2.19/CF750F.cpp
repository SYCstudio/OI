...
int main()
{
	...
	while (Case--) {
		...
		while (1) {
			...
			bool flag=0;
			for (int i=0; i<T[now].size(); i++) if (T[T[now][i]].size()==0) {
					flag=1;
					now=T[now][i];
					break;
				}
		...
		if (gr) continue;
		if (T[1].size()==3) {
			for (int i=0; i<T[1].size(); i++) if (T[T[1][i]].size()==0) {
					now=T[1][i];
					break;
				}
		...
		now=St[cnt/2+1];
		int h=H-cnt/2-1,lst=St[cnt/2+1];
		if (T[now].size()==2) {
			cout<<"! "<<now<<endl;
			fflush(stdout);
			continue;
		}
		for (int i=0; i<T[now].size(); i++) if (T[now][i]!=St[cnt/2]&&T[now][i]!=St[cnt/2+2]) {
				now=T[now][i];
				break;
			}
		if (h==1) {
		...
		while (h>3) {
			...
			for (int i=1; i<=limit; i++) {
				...
				for (int j=0; j<T[u].size(); j++) if (T[u][j]!=St[top-1]) {
						u=T[u][j];
						break;
					}
			}
			...
			if (T[u].size()==1) {
				for (int i=0; i<T[now].size(); i++) if (T[now][i]!=lst&&T[now][i]!=St[2]) {
						lst=now;
						now=T[now][i];
						break;
					}
