...
int main()
{
	...
	for (int i=1; i<n; i++) Pe[1].pb((Edge) {
		i,i+1,D2[i][1]
	}),Sml[1]+=D2[i][1];
	for (int i=2; i<=m; i++) {
		int mxsz=n+n;
		...
		for (int j=1; j<n; j++) Pe[i].pb((Edge) {
			n+j,n+j+1,D2[j][i]
		}),Sml[i]+=D2[j][i];
		for (int j=1; j<=n; j++) {
			Sml[i]+=D1[j][i-1];
	...
	for (int i=1; i<n; i++) Se[m].pb((Edge) {
		i,i+1,D2[i][m]
	}),Smr[m]+=D2[i][m];
	for (int i=m-1; i>=1; i--) {
		Se[i]=Se[i+1];
		...
		for (int j=1; j<n; j++) Se[i].pb((Edge) {
			j+n,j+1+n,D2[j][i]
		}),Smr[i]+=D2[j][i];
		for (int j=1; j<=n; j++) {
			Smr[i]+=D1[j][i];
			if (i==m-1) Se[i].pb((Edge) {
				j,j+n,D1[j][i]
			});
			else Se[i].pb((Edge) {
				j+n,j+n+n,D1[j][i]
...
{
	if (!Imp[u]) return;
	if (u!=1&&(Vt[u].size()>=3||mark[u])) E.pb((Edge) {
		anc,u,mx
	}),mx=0,anc=u;
