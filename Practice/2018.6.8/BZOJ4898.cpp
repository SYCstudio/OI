...
int n,m,K;
int In[maxN][maxK],Out[maxN][maxK];
...
bool Check(int k);
bool Spfa(int uu,int k);

int main()
{
	//freopen("1.in","r",stdin);freopen("out","w",stdout);
	scanf("%d%d%d",&n,&m,&K);
	for (int i=1;i<=n;i++) for (int j=1;j<=K;j++) scanf("%d%d",&In[i][j],&Out[i][j]);
	...
	for (int i=1;i<=n;i++)
		for (int j=1;j<=n;j++)
			if (i!=j)
			{
				Good[i][j]=0;
				for (int k=1;k<=K;k++)
					if ((In[i][k]!=-1)&&(Out[j][k]!=-1)) Good[i][j]=max(Good[i][j],(ll)Out[j][k]-In[i][k]);
				R=max(R,Good[i][j]);
			}
	do
	{
