...
int main(){
    ...
    while (Case--){
        ...
        if (K==0){
            ...
            int c=0;if (P==n) P=max(0,P-K),++c;
            if (P) c+=(P-1)/(K-1)+1;
            printf("%d\n",c);continue;
        }
        memset(Pm1,0,sizeof(Pm1));memset(Pm2,0,sizeof(Pm2));memset(Tr1,0,sizeof(Tr1));memset(Tr2,0,sizeof(Tr2));
        int im=Inv(m),im1=Inv(m+1);C[0]=Pm1[0]=1;Pm2[0]=QPow(1ll*m*im1%Mod,K);
        for (int i=1;i<=n;i++){
            C[i]=1ll*C[i-1]*(K-(i-1)+Mod)%Mod*Inv(i)%Mod;
            Pm1[i]=1ll*Pm1[i-1]*im1%Mod;Pm2[i]=1ll*Pm2[i-1]*(m+1)%Mod*im%Mod;
        }
        for (int i=0;i<=n;i++){
...
Data operator + (Data A,Data B){
    A.a=(A.a+B.a)%Mod;A.b=(A.b+B.b)%Mod;
    return A;
}
Data operator + (Data A,int B){
    A.b=(A.b+B)%Mod;return A;
}
Data operator - (Data A,Data B){
    A.a=(A.a-B.a+Mod)%Mod;A.b=(A.b-B.b+Mod)%Mod;
    return A;
}
Data operator - (Data A,int B){
    A.b=(A.b-B+Mod)%Mod;return A;
}
Data operator * (Data A,int B){
    A.a=1ll*A.a*B%Mod;A.b=1ll*A.b*B%Mod;return A;
}