...
int main()
{
	...
	while (Case--) {
		...
		memset(Mksum,0,sizeof(Mksum));
		memset(Fob,0,sizeof(Fob));
		memset(Sn,0,sizeof(Sn));
		memset(Mksz,0,sizeof(Mksz));
		memset(Fob,0,sizeof(Fob));
		for (int i=1; i<=m; i++) {
			int u,v;
			scanf("%d%d",&u,&v);
		...
		int oddcnt=0;
		for (int i=1; i<=mkcnt; i++) oddcnt+=(Mksum[i]&1);
		...
		for (int i=1; i<=n; i++)
			if (Mksz[Mark[i]]==1) Outp(oddcnt-(Col[i]=='1'),mkcnt-1,n-1,m-T[i].size());
				...
				if (oo||Fob[i]) printf("0 ");
				else Outp(oo,mkcnt-1+Sn[i],n-1,m-T[i].size());
			}
		printf("\n");
	...
	for (int i=0,sz=T[u].size(); i<sz; i++) {
		...
		if (!dfn[v]) {
			...
			if (low[v]>=dfn[u]) {
				++idcnt;
				int sv;
				do sv=St[top--],Vt[idcnt].push_back(sv),Vt[sv].push_back(idcnt);
				while (sv!=v);
				Vt[u].push_back(idcnt);
				Vt[idcnt].push_back(u);
			}
