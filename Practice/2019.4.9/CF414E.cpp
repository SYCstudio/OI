...
int main()
{
	...
	while (Q--) {
		...
		if (opt==1) {
			...
			Splay(lft[v],0);
			d2=S[lft[v]].key;
			if (Rank(lft[u])<Rank(lft[v])) l=lft[u];
			else l=lft[v];
			if (Rank(rht[u])>Rank(rht[v])) r=rht[u];
			else r=rht[v];
			l=Pre(l);
			r=Nxt(r);
			Splay(l,0);
		...
		if (opt==3) {
			...
			while (1) {
				PushDown(now);
				//cout<<"At:"<<now<<" "<<S[now].mn<<" "<<S[now].mx<<" "<<S[now].key<<endl;
...
{
	S[x].pls+=key;
	S[x].mn+=key,S[x].mx+=key,S[x].key+=key;
	return;
}
int Build(int l,int r)
{
	...
	S[mid].key=S[mid].mn=S[mid].mx=Dep[Eu[mid]];
	S[mid].eo=1;
	if (l<mid) {
