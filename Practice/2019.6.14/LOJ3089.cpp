...
int main(){
	...
	if (son[1][i]){
	    Qu.push(son[1][i]);fail[son[1][i]]=1;
	}
	else son[1][i]=1;
    while (!Qu.empty()){
	...
	//cout<<l<<" "<<r<<":"<<mx<<endl;
	if (mx>0) Ans=mid,l=mid+eps;else r=mid-eps;
    }
    Calc(Ans);
    /*
    for (int i=1;i<=n;i++){
    ...
    //*/
    int ansu=1;for (int i=1;i<=tcnt;i++) if (F[n][ansu]<F[n][i]) ansu=i;
    Outp(n,ansu);
    return 0;
}
void Calc(double val){
	...
	for (int j=1;j<=tcnt;j++)
		...
		for (int k=0;k<Alpha;k++){
		    int v=son[j][k];double d=F[i-1][j]+Key[v]-val*cnt[v];
		    if (F[i][v]<d){
		...
		int v=son[j][str[i]-'0'];double d=F[i-1][j]+Key[v]-val*cnt[v];
		if (F[i][v]<d){
		    F[i][v]=d;
		    Fr[i][v]=j;frch[i][v]=str[i]-'0';
		}
	    }
    }
    /*
...
void Outp(int d,int u){
    ...
    if (d==0) return;
    Outp(d-1,Fr[d][u]);printf("%d",frch[d][u]);
    return;
}
