...
void Build(int &x,int l,int r)
{
	...
	if (l==r) {
		S[x].M=((Mat) {
			G[Id[l]][0],G[Id[l]][0],G[Id[l]][1],-inf
		});
		return;
	}
	...
	S[x].mid=pos;
	Build(S[x].ls,l,S[x].mid);
	Build(S[x].rs,S[x].mid+1,r);
	S[x].M=S[S[x].ls].M*S[S[x].rs].M;
	return;
}
void Modify(int x,int l,int r,int pos)
{
