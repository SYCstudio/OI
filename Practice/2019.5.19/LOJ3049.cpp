...
int main()
{
	...
	while (Case--) {
		...
		SAM::Init();
		scanf("%d",&na);
		for (int i=1; i<=na; i++) scanf("%d%d",&Al[i],&Ar[i]);
		scanf("%d",&nb);
		for (int i=1; i<=nb; i++) scanf("%d%d",&Bl[i],&Br[i]);
...
void dfs_build(int x)
{
	...
	if (Ra[x].size()+Rb[x].size()) {
		int sz=Rb[x].size()-1;
		sort(Rb[x].begin(),Rb[x].end());
		sort(Ra[x].begin(),Ra[x].end());
		if (Rb[x].size()) {
			Add_Edge(Idsi[x],Rb[x][0].sd);
			Add_Edge(Rb[x][sz].sd,Idso[x]);
		}
