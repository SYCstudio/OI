...
void Work(int kth)
{
	...
	for (int i=1; i<=n; i++)
		if (Ans[i]>=1) {
			...
			if (A[i]<=A[St[p]]) Cov[++scnt]=mp(0,1);
			else Cov[++scnt]=mp(Cross(i,St[p]).floor()+1,1);
			l=2;
			r=top;
			p=1;
			while (l<=r) {
