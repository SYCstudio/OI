...
int main(){
	...
	if (c==0){
	    ...
	    assert(Mp[x][t]);assert(yt);
	    G[Mp[x][t]+1].pb(yt+1);G[yt].pb(Mp[x][t]);
	}
	else{
	    int yt=Mp[y][*lower_bound(Id[y].begin(),Id[y].end(),t)];
	    assert(Mp[x][t]);assert(yt);
	    G[Mp[x][t]].pb(yt+1);G[yt].pb(Mp[x][t]+1);
	}
    }
    ...
    while (!Qu.empty()){
	int u=Qu.front();Qu.pop();Tps[++tpscnt]=u;
	for (int i=0;i<G[u].size();i++) if ((--Dg[G[u][i]])==0) Qu.push(G[u][i]);
    }
    for (int l=1,r;l<=n;l=r+1){
	r=min(n,l+maxB-1);
	for (int i=1;i<=tcnt;i++) GET[i].reset();fob.reset();
    ...
    for (int i=1;i<=n;i++) printf("%d ",(mark[i]^1)*(n-1-Ans[i]));return 0;
}
