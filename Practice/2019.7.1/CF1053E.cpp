...
int main(){
    ...
    scanf("%d",&n);N=n+n-1;for (int i=1;i<=N;i++) scanf("%d",&seq[i]),++Cnt[seq[i]];
    for (int i=1;i<=n;i++) if (!Cnt[i]) empty.push(i);

    if (!seq[1]&&!seq[N]){
	if (empty.empty()) NO();
	seq[1]=seq[N]=empty.front();empty.pop();
    }
...
void Solve(int l,int r){
	...
	while (sz>=2&&seq[vec[sz-1]]&&(
	    ...
	    vec.pop_back();vec.pop_back();sz-=2;
	}
	vec.pb(i);
    }
    ...
    int rt=vec[0];
    deque<int> dq;for (int i=1;i+1<vec.size();i++) dq.pb(vec[i]);
    while (!dq.empty()){
	...
	if (zero==2){
	    if (empty.empty()) NO();
	    seq[u]=seq[v]=rt=empty.front();empty.pop();
	    dq.pop_front();dq.pop_back();
	}
	else if (zero==1){
	    rt=seq[u]=seq[v]=seq[u]+seq[v];dq.pop_front();dq.pop_back();
	}
	else{
	    seq[dq[dq.size()-2]]=rt;dq.pop_back();dq.pop_back();
	}
    }
    ufs[ufs_find(r)]=l;
    return;
