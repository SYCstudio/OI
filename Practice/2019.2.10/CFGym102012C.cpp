...
int main()
{
	...
	while (Case--) {
		scanf("%d",&n);
		for (int i=0; i<=n; i++) scanf("%lf",&H[i].y),H[i].x=i;
		H[n+1]=((Point) {
			n+1,0
		});
		for (int i=0; i<=n; i++) for (int j=0; j<=n; j++) D[0][i][j]=D[1][i][j]=inf;
		D[0][0][n]=0;
		Heap.push((QData) {
			0,0,n,0
		});
		mem(vis,0);
		while (!Heap.empty()) {
			int a=Heap.top().a,b=Heap.top().b,opt=Heap.top().opt;
			Heap.pop();
			if (vis[opt][a][b]) continue;
			vis[opt][a][b]=1;
			Point A,B;
			double bd=D[opt][a][b];
...
void Push(int opt,int a,int b,double cost)
{
	...
	if (D[opt][a][b]>cost) Heap.push((QData) {
		opt,a,b,D[opt][a][b]=cost
	});
	return;
}