...
int main()
{
	...
	for (int i=1; i<=n; i++) {
		int col;
		scanf("%d",&col);
		if (ColMap.count(col)==0) ColMap[col]=++colcnt;
		Col[i]=ColMap[col];
		Belong[Col[i]].push_back(i);
	}
	for (int i=1; i<=m; i++) {
		int u,v;
		scanf("%d%d",&u,&v);
	...
	for (int c=1; c<=colcnt; c++) {
		int scnt=Belong[c].size();
		for (int i=0; i<scnt; i++) Sorter[i+1]=Belong[c][i];
		...
		for (int l=1,r; l<=scnt; l=r+1) {
			r=l;
			while (r<scnt&&Bcc[Sorter[r+1]]==Bcc[Sorter[l]]) ++r;
			//cout<<"("<<l<<","<<r<<")"<<endl;
			int bcnt=0;
			for (int i=l; i<=r; i++) Bp[++bcnt]=Sorter[i];
			for (int i=l; i<r; i++) Bp[++bcnt]=LCA(Sorter[i],Sorter[i+1]);
			sort(&Bp[1],&Bp[bcnt+1],dcmp);
			bcnt=unique(&Bp[1],&Bp[bcnt+1])-Bp-1;
		...
		if (V[i]!=fa) {
			...
			if (dfn[v]==0) {
				...
				if (low[v]>=dfn[u]) {
					++idcnt;
					int p;
					do p=St[top--],::Add_Edge(idcnt,p),::Add_Edge(p,idcnt);
					while (p!=v);
					::Add_Edge(idcnt,u);
					::Add_Edge(u,idcnt);
				}
