...
int main()
{
	scanf("%s",In+1);
	int len=strlen(In+1);
	static int Bk[40],Rk[maxN],Str[maxN];
	...
	for (int i=2; i<=len; i++) printf("%d ",Sa[i]);
	printf("\n");
	for (int i=2; i<len; i++) printf("%d ",Ht[i]);
	printf("\n");
	return 0;
}
void Sais(int n,int *s)
{
	int Label[maxN],Lms[maxN],m=0,nS[maxN],tot=0;
	Label[n]=1;
	for (int i=n-1; i>=1; i--) Label[i]=(s[i]==s[i+1]?Label[i+1]:(s[i]<s[i+1]));
	for (int i=1; i<=n; i++) if (Label[i]&&!Label[i-1]) Lms[Rk[i]=++m]=i;
		else Rk[i]=-1;
	Inducesort(n,m,s,Lms,Label);
	Lms[m+1]=n+1;
	...
	if (tot==m) for (int i=1; i<=m; i++) Sa[nS[i]]=i;
	else Sais(m,nS);
