...
int main(){
    ...
    int Case,M;scanf("%d%d",&Case,&M);
    if (M==2){
	G=5;
	C1=(Data){Inv(2),Inv(10)};C2=(Data){C1.a,Mod-C1.b};
	X1=(Data){Inv(2),Inv(2)};X2=(Data){X1.a,Mod-X1.b};
    }
    else{
	G=3;
	C1=(Data){Inv(2),Inv(6)};C2=(Data){C1.a,Mod-C1.b};
	X1=(Data){2,1};X2=(Data){X1.a,Mod-X1.b};
    }
    px1[0]=px2[0]=pc1[0]=pc2[0]=(Data){1,0};
...
Data Inv(Data A){
    ...
    ic=Inv(ic);
    assert(ic>=0);
    return (Data){1ll*A.a*ic%Mod,1ll*(Mod-A.b)*ic%Mod};
}
Data operator + (Data A,int B){
    return (Data){(A.a+B)%Mod,A.b};
}
Data operator * (Data A,int B){
    return (Data){1ll*A.a*B%Mod,1ll*A.b*B%Mod};
}
Data operator + (int A,Data B){
    return B+A;
}
...
Data operator - (Data A,Data B){
    return (Data){(A.a-B.a+Mod)%Mod,(A.b-B.b+Mod)%Mod};
}
Data operator * (Data A,Data B){
    return (Data){(1ll*A.a*B.a%Mod+1ll*G*A.b%Mod*B.b%Mod)%Mod,(1ll*A.a*B.b%Mod+1ll*A.b*B.a%Mod)%Mod};
}
int Calc(long long up,int K){
    static Data f[maxK];static int poly[maxK];
    for (int i=0;i<=K;i++) f[i]=(Data){0,0};memset(poly,0,sizeof(poly));
    for (int i=0;i<=K;i++)
	for (int j=0;j<=i;j++){
	    Data mul=px1[j]*px2[i-j];
	    
	    if (mul.a==1&&mul.b==0) mul=(Data){up%Mod,0};
	    else mul=(mul-QPow(mul,up+1))*Inv((Data){1,0}-mul);
    ...
    int fc=1;for (int i=1;i<=K;i++) fc=1ll*fc*i%Mod;
    int ifc=Inv(fc);
