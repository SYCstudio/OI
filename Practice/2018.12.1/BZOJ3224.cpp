...
int main(){
	...
	while (n--){
		...
		if (opt==3){
			int r1,r2,r3;Split(root,x,r1,r3);Split(r1,x-1,r1,r2);
			//Outp(r1);cout<<endl;Outp(r2);cout<<endl;
			printf("%d\n",T[r1].size);
			root=Merge(Merge(r1,r2),r3);
		}
		if (opt==4){
		...
		//cout<<"root:"<<root<<endl;Outp(root);cout<<endl;
	}
	return 0;
}
...
void Outp(int u){
	//cout<<u<<"["<<T[u].key<<"] "<<T[u].ch[0]<<" "<<T[u].ch[1]<<" "<<T[u].size<<" "<<T[u].cnt<<endl;
