...
public:
	...
	{
		if (p==0) return;
		while (p<=sz) B[p]+=k,p+=(p)&(-p);
		return;
	}
	ll Sum(int p)
	{
		ll ret=0;
		while (p) ret+=B[p],p-=(p)&(-p);
		return ret;
	}
...
public:
	...
	void dfs(int u,int fa)
	{
		...
		dfn[u]=++dfncnt;
		for (int i=Hd[u]; i!=-1; i=Nt[i]) if (V[i]!=fa) dfs(V[i],u);
		rht[u]=dfncnt;
		return;
	}
	void Plus(int u,int v,ll c)
	{
		B.Add(dfn[u],c),B.Add(dfn[v],c);
		B.Add(dfn[rt[u]],-c);
		return;
	}
	ll Query(int x)
...
void Divide(int d,int l,int r)
{
	...
	for (int p=1; p<=m; p++) {
		for (int i=1; i<=m; i++) for (int j=l; j<=r; j++) D[i][j]=INF,vis[i][j]=0;
		while (!H.empty()) H.pop();
		D[p][mid]=0;
		H.push(mp(0ll,mp(p,mid)));
		From[p][mid]=-1;
		while (!H.empty()) {
...
ll Query(int d,int l,int r,int p,int u)
{
	...
	ll ret=0;
	for (int i=0; i<m; i++) ret+=S[d][i].Query(u);
	if (l==r) return ret;
	int mid=(l+r)>>1;
