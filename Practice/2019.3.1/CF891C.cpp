...
int main()
{
	...
	while (Q--) {
		...
		bool flag=1;
		sort(&Sorter[1],&Sorter[K+1],cmp1);
		for (int l=1,r; l<=K&&flag; l=r+1) {
			r=l;
			while (r<K&&Sorter[r+1].w==Sorter[l].w) ++r;
			for (int i=l; i<=r; i++) ufs[Sorter[i].u]=Sorter[i].u,ufs[Sorter[i].v]=Sorter[i].v;
			for (int i=l; i<=r&&flag; i++) if (find(Sorter[i].u)==find(Sorter[i].v)) {
					flag=0;
					break;
				} else ufs[find(Sorter[i].u)]=find(Sorter[i].v);
		}
