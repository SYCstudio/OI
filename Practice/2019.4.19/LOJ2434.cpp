...
void dfs_init(int u,int fa){
    VSum[u]=0;ll mx=Val[u];int mxid=u;
    for (int i=0,sz=It[u].size();i<sz;i++){
	int v=It[u][i];if (v==fa) continue;
	S[v].fa=u;dfs_init(v,u);VSum[u]+=Sum[v];
...
void Update(int x){
    Sum[x]=Sum[S[x].ch[0]]+Sum[S[x].ch[1]]+VSum[x]+Val[x];return;
}
bool Isroot(int x){
    int fa=S[x].fa;
...
void Access(int x,ll key){
    int lstx=0;
    while (x){
	Splay(x);Ans=Ans-Ras[x];
	Sum[x]+=key;
	if (lstx) VSum[x]+=key;else Val[x]+=key;
	...
	if (Sum[S[x].ch[1]]+Sum[S[x].ch[1]]>rS) Ras[x]=2ll*(rS-Sum[S[x].ch[1]]);
	    ...
	    VSum[x]+=Sum[S[x].ch[1]];S[x].ch[1]=0;
	    if (Val[x]+Val[x]>rS) Ras[x]=2ll*(rS-Val[x]);
	    else Ras[x]=rS-1;
	}
	Ans=Ans+Ras[x];lstx=x;x=S[x].fa;
    }
    return;
}
