...
int main(){
    ...
    Dep[1]=1;dfs_init(1);
    W=Top[1];dfs_build(1,1);
    for (int i=0;i<LEAF;i++) Lkey[i].ft=abs(W-Lkey[i].sd);sort(Lkey.begin(),Lkey.end());
...
void dfs_init(int u){
    if (Dep[u]&1) Top[u]=-inf;else Top[u]=inf;
    Sz[u]=1;Mx[u]=0;
    for (int i=0,sz=V[u].size();i<sz;i++)
	if (V[u][i]!=Fa[u]){
	    Fa[V[u][i]]=u;Dep[V[u][i]]=Dep[u]+1;dfs_init(V[u][i]);Sz[u]+=Sz[V[u][i]];
	    if (Dep[u]&1) Top[u]=max(Top[u],Top[V[u][i]]);else Top[u]=min(Top[u],Top[V[u][i]]);
...
void dfs_build(int u,int top){
    ...
    if (Mx[u]){
	dfs_build(Mx[u],top);F[u]=G[u]=ZeroData(1);
	for (int i=0,sz=V[u].size();i<sz;i++)
	    if (V[u][i]!=Fa[u]&&V[u][i]!=Mx[u]){
		dfs_build(V[u][i],V[u][i]);
		Matrix K=Query(1,1,n,dfn[V[u][i]],rht[V[u][i]]);
		F[u]=F[u]*K.M[1][0];G[u]=G[u]*K.M[2][0];
	    }
    }
    else{
	if (u>W) F[u]=ZeroData(1);if (u<W) G[u]=ZeroData(1);
	rht[Top[u]]=dfncnt;
...
void Mul(int &x,int y){
    x=1ll*x*y%Mod;return;
}
int QPow(int x,int cnt){
    int ret=1;
    while (cnt){
	if (cnt&1) Mul(ret,x);Mul(x,x);cnt>>=1;
    }
    return ret;
}
ZeroData operator * (ZeroData A,int B){
    if (B==0) ++A.zero;else Mul(A.real,B);
    return A;
}
ZeroData operator / (ZeroData A,int B){
    if (B==0) --A.zero;else Mul(A.real,Inv(B));
    return A;
}
Matrix operator * (const Matrix A,const Matrix B){
...
void Modify(int x,int l,int r,int p){
	...
	if (Mx[p]){
	    ...
	    S[x].M[0][0]=1;S[x].M[1][0]=F[p].r();S[x].M[2][0]=G[p].r();
	}
	return;
    }
    int mid=(l+r)>>1;
    if (p<=mid) Modify(ls,l,mid,p);else Modify(rs,mid+1,r,p);
...
void Skip(int u){
    if (u<W) F[u]=ZeroData(inv2);else G[u]=ZeroData(inv2);
    while (u){
	int top=Top[u],fa=Fa[top];
	Matrix K=Query(1,1,n,dfn[top],rht[top]);
	if (fa){
	    F[fa]=F[fa]/K.M[1][0];G[fa]=G[fa]/K.M[2][0];
	}
	Modify(1,1,n,dfn[u]);K=Query(1,1,n,dfn[top],rht[top]);
	if (fa){
	    F[fa]=F[fa]*K.M[1][0];G[fa]=G[fa]*K.M[2][0];
	}
	u=fa;
