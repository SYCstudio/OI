...
int main()
{
	...
	for (int i=1; i<=n+1; i++) {
		int mul=1,invi=QPow(Mod-i,Mod-2);
		for (int j=1; j<=n+1; j++) if (i!=j) mul=1ll*mul*(i-j+Mod)%Mod;
		mul=QPow(mul,Mod-2);
...
{
	return ((Data) {
		1ll*A.a*B.a%Mod,(1ll*A.b*B.a%Mod+B.b)%Mod,(1ll*A.a*B.c%Mod+A.c)%Mod,(1ll*A.b*B.c%Mod+A.d+B.d)%Mod
	});
}
void PushDown(int x)
{
...
int Merge(int x,int y)
{
	...
	if (!S[y].ls&&!S[y].rs) {
		S[x].lazy=S[x].lazy+((Data) {
			S[y].lazy.b,0,0,S[y].lazy.d
		});
		return x;
	}
	PushDown(x);
